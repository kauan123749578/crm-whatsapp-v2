...
let WhatsAppService = WhatsAppService_1 = class WhatsAppService {
    ...
    chatsInFlight = new Map();
    constructor(prisma) {
        this.prisma = prisma;
    }
    async ensureWWebJS(inst) {
        ...
        try {
            ...
            let hasStore = false;
            try {
                hasStore = await page.evaluate(() => typeof window.Store !== 'undefined');
            }
            catch (e) {
                this.logger.warn(`[${inst.id}] Erro ao verificar Store: ${e.message}`);
            }
            ...
            let hasWWebJS = false;
            try {
                hasWWebJS = await page.evaluate(() => typeof window.WWebJS !== 'undefined');
            }
            catch (e) {
                this.logger.warn(`[${inst.id}] Erro ao verificar WWebJS: ${e.message}`);
            }
            if (!hasWWebJS) {
                ...
                try {
                    ...
                    this.logger.error(`[${inst.id}] Falha ao reinjetar WWebJS: ${e.message}`);
                    throw new Error(`Falha ao reinjetar WWebJS: ${e.message}`);
                }
            }
            // Garantir que o método getChats existe
            const hasGetChats = await page.evaluate(() => typeof window.WWebJS?.getChats === 'function');
            if (!hasGetChats) {
            ...
            this.logger.error(`[${inst.id}] ensureWWebJS falhou: ${e.message}`);
            throw e;
        }
    }
    async restartInstance(instanceId) {
        ...
        try {
            ...
            if (inst.client) {
                try {
                    ...
                    if (emitFunc) {
                        ...
                        inst.client.on('disconnected', async (reason) => {
                            inst.qr = null;
                            inst.readyAt = null;
                            inst.chatsCache = null;
                    ...
                    await inst.client.initialize();
                    this.logger.log(`[${instanceId}] Instância reinicializada com sucesso`);
                }
                catch (reInitError) {
                    this.logger.warn(`[${instanceId}] Falha ao reinicializar, tentando destroy + recreate: ${reInitError.message}`);
                    ...
                    try {
                        ...
                        this.logger.warn(`[${instanceId}] Erro ao destruir client: ${destroyError.message}`);
                    }
                    // Recriar o client (vai usar LocalAuth, pode pedir QR se necessário)
                    const dataPath = (process.env.WA_DATA_PATH?.trim() ||
                        node_path_1.default.join(process.cwd(), '.wwebjs_auth'));
            ...
            this.logger.error(`[${instanceId}] Erro ao reiniciar instância: ${e.message}`);
            inst.status = 'error';
            throw e;
        }
    }
    ...
    async getOrCreate(id, emit) {
        if (this.instances.has(id))
            return this.instances.get(id);
        if (this.creating.has(id))
            return await this.creating.get(id);
        const promise = (async () => {
            ...
            const setStatus = async (status, message) => {
                ...
                const payload = { instanceId: id, status, message };
            ...
            runtime.client.on('auth_failure', async (msg) => {
            ...
            runtime.client.on('message', async (msg) => {
                try {
                    ...
                    this.logger.warn(`Falha ao persistir mensagem: ${e.message}`);
                }
            });
    ...
    async waitUntilReady(instanceId, timeoutMs = 30000) {
    ...
    async getChats(instanceId) {
        const inst = this.instances.get(instanceId);
        if (!inst)
            throw new Error('Instância não existe (ainda)');
            ...
            try {
                ...
                for (let attempt = 1; attempt <= 3; attempt++) {
                    try {
                        ...
                        if (this.dbEnabled) {
                            await this.prisma.$transaction(mapped.map((c) => this.prisma.chat.upsert({
                                where: { id: c.id },
                                update: {
                            ...
                            const dbChats = await this.prisma.chat.findMany({
                                where: { instanceId, id: { in: mapped.map((c) => c.id) } },
                                select: { id: true, tags: true, stage: true, name: true }
                            });
                            const dbChatsMap = new Map(dbChats.map((db) => [db.id, db]));
                        ...
                        if (/timed out|protocolTimeout|detached Frame|Target closed|Protocol error|Execution context|getChats|WWebJS|Store/i.test(msg)) {
                            ...
                            if (/detached Frame|Target closed|Execution context|Store não encontrado|getChats/i.test(msg)) {
                                try {
                                    ...
                                    this.logger.warn(`restartInstance falhou: ${String(re?.message || re)}`);
                                }
                            }
    ...
    async getMessages(instanceId, chatId, limit = 50) {
        const inst = this.instances.get(instanceId);
        if (!inst)
            throw new Error('Instância não existe (ainda)');
        try {
            ...
            if (this.dbEnabled) {
                await this.prisma.$transaction(mapped.map((m) => this.prisma.message.upsert({
                    where: { id: m.id },
                    update: {},
                    create: {
                        id: m.id,
    ...
    async sendMessage(instanceId, chatId, text, mediaUrl, mediaType, userId) {
        const inst = this.instances.get(instanceId);
        if (!inst)
            throw new Error('Instância não existe (ainda)');
    ...
    async updateChatTags(instanceId, chatId, tags, userId) {
        if (!this.dbEnabled) {
    ...
    async updateChatStage(instanceId, chatId, stage, userId) {
        if (!this.dbEnabled) {
            // Modo dev: retornar dados mockados
            return {
                id: chatId,
    ...
    async getContactInfo(instanceId, chatId) {
        const inst = this.instances.get(instanceId);
        if (!inst)
            throw new Error('Instância não existe (ainda)');
        try {
    ...
    async getMetrics() {
        ...
        try {
            ...
            const userIds = chatsByUser.map((c) => c.userId).filter((id) => !!id);
            ...
            return {
                ...
                totalMessages,
                totalUsers,
                chatsByStage: Object.fromEntries(chatsByStage.map((s) => [s.stage || 'Sem estágio', s._count.id])),
                chatsByUser: Object.fromEntries(chatsByUser.map((c) => [userMap.get(c.userId) || 'Sem atendente', c._count.id])),
